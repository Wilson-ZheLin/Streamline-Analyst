...
from src.plot import list_all, correlation_matrix
...
from src.model_service import split_data, save_model, calculate_f1_score
...
def regression_model_pipeline(DF, API_KEY, GPT_MODEL):
    ...
    if st.session_state['start_training']:
        with st.container():
            st.header("Modeling")
            X_train_res, Y_train_res = select_Y(st.session_state.df_pca, selected_Y)
