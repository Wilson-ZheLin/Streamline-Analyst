...
column_description = """
Ngày: Kiểu dữ liệu ngày không phải số, Ngày mà thông tin về bất động sản được ghi nhận.
Địa chỉ: Kiểu dữ liệu không phải số, Địa chỉ chi tiết của bất động sản, bao gồm tên đường, phường/xã, quận/huyện.
Quận: Kiểu dữ liệu không phải số, Đơn vị hành chính cấp hành chính lớn hơn phường/xã, nơi bất động sản đặt tại.
Huyện: Kiểu dữ liệu không phải số, Đơn vị hành chính lớn hơn phường/xã nơi bất động sản đặt tại.
Loại hình nhà ở: Kiểu dữ liệu không phải số, Mô tả loại hình của bất động sản, ví dụ nhà ngõ, nhà mặt phố, biệt thự, chung cư, v.v.
Giấy tờ pháp lý: Kiểu dữ liệu không phải số, Trạng thái pháp lý của bất động sản, bao gồm những loại như đã có sổ đỏ, đang chờ cấp sổ, hoặc không rõ ràng.
Số tầng: Kiểu dữ liệu số, Số tầng của bất động sản, bao gồm cả tầng hầm và tầng trệt nếu có.
Số phòng ngủ: Kiểu dữ liệu số,  Số lượng phòng ngủ trong bất động sản.
Diện tích: Kiểu dữ liệu số,  Diện tích tổng thể của bất động sản, tính bằng mét vuông (m²).
Dài: Kiểu dữ liệu số,  Chiều dài của bất động sản, tính bằng mét (m).
Rộng: Kiểu dữ liệu số,  Chiều rộng của bất động sản, tính bằng mét (m).
Giá/m2: Kiểu dữ liệu số,  Giá bán hoặc giá cho thuê trung bình của một mét vuông bất động sản, tính bằng đơn vị tiền tệ (ví dụ như VND).
"""

chart_description = """
Scatter plot: Yêu cầu 2 thuộc tính.
Heatmap: Các thuộc tính không phải số.
Boxplot: Các thuộc tính không phải số.
Violin plot: Các thuộc tính không phải số.
Strip plot: Các thuộc tính không phải số.
Line plot: Yêu cầu 2 thuộc tính, và các thuộc tính phải là số.
"""
...
def predict_visualization_chart(param, template, attrs_value):
    ...
    prompt_template = PromptTemplate(input_variables=[
        "shape_info", "head_info", "nunique_info", "description_info", "question", "column_description","chart_description", "attrs"], template=template)
    summary_prompt = prompt_template.format(shape_info=shape_info, head_info=head_info, nunique_info=nunique_info,
                                            description_info=description_info, column_description=column_description,chart_description=chart_description, attrs=attrs_value["attrs"], question=question)
